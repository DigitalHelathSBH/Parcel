"""
รายงานครุภัณฑ์ต่ำกว่าเกณฑ์ (ราคาต่ำกว่า 10,000 บาท) รายละเอียด ตามแหล่งเงิน
เลย์เอาต์เดียวกับรายงานค่าเสื่อมราคารายละเอียดตามแหล่งเงิน (depreciation_builder.build_detail_excel_bytes)
แต่ไม่มีคอลัมน์ค่าเสื่อม เพราะครุภัณฑ์กลุ่มนี้ไม่ขึ้นบัญชีสินทรัพย์ (DEPREGROUP='2') จึงไม่มีการคิดค่าเสื่อมราคา
"""
import io
...
def fetch_low_value_data(as_of_date, division=None, dept=None, section=None) -> pd.DataFrame:
    conditions = [
        "cm.PRICE < ?",
        "cm.ACQDATETIME <= ?",
...
_COLUMNS = ["รหัส", "รายการ", "วันเดือนปีที่ได้มา", "จำนวน", "ราคาต่อหน่วย", "มูลค่ารวม"]
_WIDTHS = [16, 40, 14, 8, 14, 16]


def build_excel_bytes(df: pd.DataFrame, as_of_date) -> bytes:
    wb = Workbook()
    ws = wb.active
    ws.title = "ครุภัณฑ์ต่ำกว่าเกณฑ์"
    ...
    c = ws.cell(1, 1, f"รายงานครุภัณฑ์ต่ำกว่าเกณฑ์ (ราคาต่ำกว่า {LOW_VALUE_THRESHOLD:,} บาท) รายละเอียด ตามแหล่งเงิน")
    c.font = Font(name=THAI_FONT, size=18, bold=True)
    c.alignment = Alignment(horizontal="center")

    ws.merge_cells(start_row=2, start_column=1, end_row=2, end_column=n_cols)
    c2 = ws.cell(2, 1, f"ข้อมูล ณ วันที่ {to_thai_date(as_of_date)}")
    ...
    for budget_label, bdf in df.groupby("BudgetLabel", sort=False):
        ...
        for art_label, adf in bdf.groupby("ArticleGroupLabel", sort=False):
            ...
            for _, r in adf.iterrows():
                values = [
                    r["ASSETCODE"], r["AssetName"], r["AcqDateThai"], r["QTY"], r["PRICE"], r["TotalValue"],
                ]
            ...
            c = ws.cell(row, 6, sub_total_value)
    ...
    gc2 = ws.cell(row, 6, grand_total_value)
